...
Sample_Name = "bNTI_16S_OTU_Synoptic_CB_transition_allsites_16S_100" # Input sample name
Factor = "transect"
Level = "transition"
...
for(rep in range){
  ...
  if(!dir.exists(paste0("/compass/fme200002/16S-FTICR/transition-CB/", Sample_Name, "/"))){
    dir.create(paste0("/compass/fme200002/16S-FTICR/transition-CB/", Sample_Name, "/"))
  }
  
  if(!dir.exists(paste0("/compass/fme200002/16S-FTICR/transition-CB/", Sample_Name, "/", Factor, "-", Level))){
    dir.create(paste0("/compass/fme200002/16S-FTICR/transition-CB/", Sample_Name, "/", Factor, "-", Level))
  }
  
  write.csv(null.by.samp, paste0("/compass/fme200002/16S-FTICR/transition-CB/", Sample_Name, "/", Factor, "-", Level, "/", 
