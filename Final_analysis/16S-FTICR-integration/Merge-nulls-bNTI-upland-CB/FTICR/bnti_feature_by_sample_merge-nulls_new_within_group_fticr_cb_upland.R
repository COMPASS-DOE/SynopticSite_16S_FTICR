Sample_Name = "bNTI_Feat_OTU_Synoptic_CB_upland_allsites_FTICR_100" # Input sample name
Factor = "transect"
Level = "upland"
...
setwd("/Users/band985/Library/CloudStorage/OneDrive-PNNL/Documents/16S synoptic site/16S-FTICR-combined-analysis/Analysis after first meeting/Cleaned-and-organized/Create nulls - FTICR - CB/upland CB/100 perm")
data<- read.csv("icr_wide_cb.csv", row.names=1, header=TRUE)
data<- t(data)
tree = read.tree("COMPASS_FTICR_Synoptic_Final.tre")
meta = read.csv("sample_metadata_cb.csv")
...
files = list.files(path = "transect-upland-100/",
...
feat.by.samp<- read.csv("bNTI_Feat_OTU_Synoptic_CB_upland_allsites_FTICR_100_bNTI_feature_by_samp_transect-upland_100rep.csv", row.names=1)
...
specified_labels <- c("C19H22O12","C20H14O15","C19H14O12","C21H18O13","C19H14O10","C19H20O11","C24H16O14","C24H20O15","C20H12O14","C27H30O11","C25H24O11","C27H34O12","C23H28O8",
                      "C20H22O14",
                      "C22H14O15",
                      "C24H22O10",
                      "C25H22O15",
...
label_replacements <- data.frame(
  ...
  display_label = c("unsaturated/lignin",
                    "aromatic",
                    "aromatic",
                    "unsaturated/lignin",
                    "aromatic",
...
p <- ggtree(tree) #checked using as.tibble that label is actually labeled as "label"
...
specified_labels <- c("C19H22O12","C20H14O15","C19H14O12","C21H18O13","C19H14O10","C19H20O11","C24H16O14","C24H20O15","C20H12O14","C27H30O11","C25H24O11","C27H34O12","C23H28O8",
                     ...
                     "C25H22O15",
                      ...
                      "C22H30O7",
                      "C20H20O7")
highlight_labels <- c("unsaturated/lignin",
                      "aromatic",
                      "aromatic",
                      "unsaturated/lignin",
                      "aromatic",
...
##not using labels on the tree as its getting hard to read so many
...
p4
ggsave("bnti_cb_upland_all_sites_fticr_100perm_06.05.25_labels_ggrepel.TIFF", plot=p4, dpi=300, height=10, width=20, units=c("in"))
...
ggsave("bnti_cb_upland_all_sites_fticr_100perm_03.13.26_labels_ggrepel.TIFF", plot=p4, dpi=300, height=10, width=20, units=c("in"))


##merge the dataframe with bnti values with the sample metadata
metadata<- read.csv("sample_metadata_cb.csv")
tax<- read.csv("icr_meta.csv")
feat.samp.frame.metadata<- feat.samp.frame_labeled %>% left_join(metadata, by="Sample")
feat.samp.frame.metadata <- feat.samp.frame.metadata %>% mutate(formula=Member)
feat.samp.frame.metadata.tax<- feat.samp.frame.metadata %>% left_join(tax, by="formula")
...
feat.metadata.tax.mean.bnti<- feat.samp.frame.metadata.tax %>% group_by(formula) %>%
  mutate(mean = mean(Value_final)) %>%
  ungroup() # It's good practice to ungroup after the operation

write.csv(feat.samp.frame.metadata, "feat.samp.frame.metadata.cb.upland.icr.csv")
write.csv(feat.samp.frame.metadata.tax, "feat.samp.frame.metadata.tax.cb.upland.icr.csv")
write.csv(feat.metadata.tax.mean.bnti, "feat.metadata.tax.mean.bnti.icr.csv")



##keep only the orders in the network 
feat.metadata.tax.mean.bnti.subset <- feat.metadata.tax.mean.bnti %>% filter(Class==c("unsaturated/lignin",
                                                                                      "aromatic",
                                                                                      "aromatic",
                                                                                      "unsaturated/lignin",
                                                                                      "aromatic",
...
formula_to_label <- c("C19H22O12","C20H14O15","C19H14O12","C21H18O13","C19H14O10","C19H20O11","C24H16O14","C24H20O15","C20H12O14","C27H30O11","C25H24O11","C27H34O12","C23H28O8",
                   "C20H22O14",
                   "C22H14O15",
                   "C24H22O10",
                   "C25H22O15",
...
data_for_plotting <- feat.metadata.tax.mean.bnti.subset %>%
  mutate(
    ...
    point_label = if_else(
      # Condition: is the formula in our target list?
      formula %in% formula_to_label, 
      # If TRUE, use the formula as the label
...
write.csv(plot_summary_data, "plot_summary_data_16s_icr_upland.csv")

bnti_plot<- ggplot(plot_summary_data, aes(Class, 
...
ggsave("bnti_cb_upland_all_sites_fticr_100perm_03.12.26_points.TIFF", plot=bnti_plot, dpi=300, height=5, width=6, units=c("in"))
