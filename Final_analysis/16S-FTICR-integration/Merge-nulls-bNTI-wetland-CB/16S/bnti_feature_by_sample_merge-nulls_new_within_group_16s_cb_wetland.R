Sample_Name = "bNTI_16S_OTU_Synoptic_CB_wetland_allsites_16S_100" # Input sample name
Factor = "transect"
Level = "wetland"
...
data<- read.csv("OTU_CB_rarefied20k.csv", row.names=1, header=TRUE)
#data<- t(data)
tree = read.tree("tree_cb_full_rooted_16s.nwk")
...
feat.by.samp<- read.csv("bNTI_16S_OTU_Synoptic_CB_wetland_allsites_16S_100_bNTI_feature_by_samp_transect-wetland_100rep.csv", row.names=1)
...
specified_labels <- c("3848c62a85b8fba27e239e3d6710bb76","db5845126f0911a9cbbb4933789b16ff")
label_replacements <- data.frame(
  original_label = specified_labels,
  display_label = c("Rhizobiales", "Desulfobacterales")
)
...
specified_labels <- c("3848c62a85b8fba27e239e3d6710bb76","db5845126f0911a9cbbb4933789b16ff")
highlight_labels <- c("Rhizobiales", "Desulfobacterales")
...
ggsave("bnti_cb_wetland_all_sites_16s_100perm_06.05.25_labels_ggrepel.TIFF", plot=p4, dpi=300, height=10, width=20, units=c("in"))
...
ggsave("bnti_cb_wetland_all_sites_16s_100perm_03.12.26_labels_ggrepel.TIFF", plot=p4, dpi=300, height=10, width=20, units=c("in"))
...
write.csv(feat.samp.frame.metadata, "feat.samp.frame.metadata.cb.wetland.csv")
write.csv(feat.samp.frame.metadata.tax, "feat.samp.frame.metadata.tax.cb.wetland.csv")
write.csv(feat.metadata.tax.mean.bnti, "feat.metadata.tax.mean.bnti.cb.wetland.csv")



##keep only the orders in the network 
feat.metadata.tax.mean.bnti.subset <- feat.metadata.tax.mean.bnti %>% filter(Order==c("o__Rhizobiales", "o__Desulfobacterales"))
feat.metadata.tax.mean.bnti.subset

##i want to label the specific otus in the network
otus_to_label <- c("3848c62a85b8fba27e239e3d6710bb76","db5845126f0911a9cbbb4933789b16ff")
...
write.csv(plot_summary_data, "plot_summary_data_16s_cb_wetland.csv")
...
ggsave("bnti_cb_wetland_all_sites_16s_100perm_03.12.26_points.TIFF", plot=bnti_plot, dpi=300, height=5, width=6, units=c("in"))
