...
#Figure 
ggsave(filename="pcoa-cb-wle-theme-classic-07.02.24.TIFF", plot=pcoa_cb_wle, width=8, height=6, units="in", dpi=300)
...
alphadiv_rich<- alphadiv_rich %>% mutate(transect1=ifelse(transect=="wetland-center"|transect=="wetland-transition-edge", "wetland", transect))
...
p<- ggplot(df_new, aes(transect1, 
                       mean, 
                       colour = transect1)) +
  geom_boxplot(aes(color=transect1, shape=site),outlier.shape = NA,
               fill = "white", 
               outlier.colour = NA, 
               position = position_dodge(width = 0.9),
               show_guide = FALSE ) + 
  geom_point(aes(shape=site, color=transect1),size=2 ,position = position_jitterdodge()) +
  theme_classic()+ 
  scale_color_manual(values=c("#CC79A7",  "#E69F00", "#0072B2"))+
  ...
  geom_text(data=summarized, aes(x=summarized$transect1, y =1888, 
  ...
  theme( axis.text.y = element_text(size = 16, colour = "black"), 
         ...
         legend.text = element_text(size = 12, colour = "black")) +
  guides(linetype = guide_legend(override.aes= list(color = "black")), color= guide_legend(order = 1)) +
  xlab("Transect") +labs(color="transect")#+geom_ribbon(aes(ymin=3.013662, ymax=512.018351),alpha=0.5, show.legend=FALSE)+ 
...
#sample_data() Sample Data:       [ 71 samples by 28 sample variables ]##28 after gwc is removed
#tax_table()   Taxonomy Table:    [ 19262 taxa by 8 taxonomic ranks ]
...
cap_plot<- ggplot(data=cap.ord.df, aes(x=CAP1, y=CAP2)) +
  geom_point(aes(color=transect.x, shape=site.x), size=3)+
  scale_color_manual(values=c("#CC79A7","#E69F00","#0072B2", "#CC6677"))+
  scale_shape_manual(values=c("MSM"=15, "GWI"=16, "GCREW"=17))+ ##change site names for CB
  labs(x=paste("CAP1 (", fracvar[1], "%)", sep=""),y=paste("CAP2 (", fracvar[2], "%)", sep=""))+
  labs(shape="site", fill="transect")+theme_classic()
...
otu_final<- read.csv("OTU_CB_rarefied20k.csv", row.names=1) ##change depending on region, all or cb or wle
View(otu_final)
...
marker_wle<-marker_table(lefse_transect_wle) ##24 markers at genus level for wle
...
# bar plot
plot_effect_size<-plot_ef_bar(lefse_transect_cb)
plot_effect_size
...
plot<- ggplot(sigtab_thres, aes(x=Genus, y=log2FoldChange, color=Phylum)) + 
  #scale_x_reordered() +
  geom_point(size=3) + scale_color_manual(values=as.vector(pal12))+ 
  theme_classic()+
...
##Final Figure
ggsave(filename = "Deseq2_CB_WLE_enriched_depleted_inCB_sigtab_thres_7_07.09.24.tiff", plot = plot,
       width = 35,
...
###############################
###did not use the chunk below to merge to wle otu table in the final analysis, this was test code
...
##class
##the psmelt and agglom is reducing the read count per sample slightly, total reads is not 117*20000=2340000 but 2286176
...
plot<-gg+scale_color_manual(values=as.vector(palette_new25))
