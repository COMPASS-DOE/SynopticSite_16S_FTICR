...
author: "Marta Nierychlo and Morten Simonsen Dueholm" #(need to cite)
date: "2021-08-25"
...
d13 <- amp_load(otutab = otutab, 
                taxonomy = taxonomy,
                metadata = metadata)

d13n <- amp_subset_samples(d13, minreads = 20000, normalise = TRUE)

#Subset 
d13nAS4PT <- amp_subset_samples(d13n, region == "Lake Erie ") #change location as needed
...
ggV13_1 <- gg[(gg$nCore >= n_distinct(gg$sample.id)*0.2),] %>%
  group_by(Genus) %>%
...
ggV13_2 <- gg[(gg$nCore >= n_distinct(gg$sample.id)*0.5),] %>%
  group_by(Genus) %>%
  summarise(mean_abu = mean(Genusprop)) %>%
  arrange(desc(mean_abu))
...
ggV13_3 <- gg[(gg$nCore >= n_distinct(gg$sample.id)*0.8),] %>%
  group_by(Genus) %>%
  summarise(mean_abu = mean(Genusprop)) %>%
  arrange(desc(mean_abu))
...
V13_core<- V13_core%>% mutate(type=ifelse(V4==1, "Cond.Rare.Abundant", "Core"))
write.csv(V13_core, "Core_CondAbun_genera_WLE_checkingcorrectness.csv")


###Chesapeake bay

#Subset 
d13nAS4PT <- amp_subset_samples(d13n, region == "Chesapeake Bay") #change location as needed  
...
V13_core<- V13_core%>% mutate(type=ifelse(V4==1, "Cond.Rare.Abundant", "Core"))
write.csv(V13_core, "Core_CondAbun_genera_CB_checkingcorrectness.csv")
...
core_cb_edit<- core_cb_edit %>% mutate(Genus_old=Genus) %>% select(!Genus)
core_cb_edit<- core_cb_edit %>% mutate(Genus= Genus_new) %>% select (!Genus_new)
...
core_wle_edit<- core_wle_edit %>% mutate(Genus_old=Genus) %>% select(!Genus)
core_wle_edit<- core_wle_edit %>% mutate(Genus= Genus_new) %>% select (!Genus_new)
...
ll<-c("0.120","0.100","0.050","0.010") # labels. ##all samples representation checked for bean planted drought and watered such that the range does not miss any samples
